...
func (c cameraRepo) Create(ctx context.Context, camera models.CameraBase) (int, error) {
	...
	cameraQueue := `INSERT INTO cameras (type, description, coordinates)
					VALUES ($1, $2, $3) RETURNING id;`

	coordinates := fmt.Sprintf("%g,%g", camera.Coordinates[0], camera.Coordinates[1])

	err = tx.QueryRowContext(ctx, cameraQueue, camera.Type, camera.Description, coordinates).Scan(&createdCameraID)
	if err != nil {
